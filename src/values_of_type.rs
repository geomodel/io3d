...
//  //  //  //  //  //  //  //
#[allow(dead_code)]
fn read_values<R, T>(reader: &mut BufReader<R>, values_number: usize) -> Result<Vec<T>>
...
mod read_values_of_type {
    ...
    fn no_values_error() {
        ...
        let values = read_values::<&[u8], i16>(&mut reader, 1);
    ...
    fn integer_values() -> Result<()> {
        let s = "\n\n\n-999\n\n\n\n1\n0\n5\nunreachable\n";
        let mut reader = BufReader::new( s.as_bytes() );
        let values = read_values::<&[u8], i16>(&mut reader, 4)?;
        assert!( values.len() == 4 );
        assert!( values[0] == -999 );
        assert!( values[1] == 1 );
        assert!( values[2] == 0 );
    ...
    fn countinues_values() -> Result<()> {
        let s = "\n\n\n-999\n\n\n\n1.0\n0.3\n5\nunreachable\n";
        let mut reader = BufReader::new( s.as_bytes() );
        let values = read_values::<&[u8], f32>(&mut reader, 4)?;
        assert!( values.len() == 4 );
        assert!( values[0] == -999_f32 );
        assert!( values[1] == 1_f32 );
        assert!( values[2] == 0.3_f32 );
        assert!( values[3] == 5_f32 );
        Ok(())
    }
}
