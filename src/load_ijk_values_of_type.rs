...
where R: std::io::Read, T: std::str::FromStr
{
    ...
    loop {
        ...
        if parsed_line.len() < 4 {
            return Err(anyhow::anyhow!("Unable to parse <{}> as i-j-k-value #{}", line, counter+1));
        };
        let Ok(i) = parsed_line[0].parse::<u16>() else {
            return Err(anyhow::anyhow!("Unable to parse <{}> as I-coordinate #{}", parsed_line[0], counter+1));
        };
        let Ok(j) = parsed_line[1].parse::<u16>() else {
            return Err(anyhow::anyhow!("Unable to parse <{}> as J-coordinate #{}", parsed_line[1], counter+1));
        };
        let Ok(k) = parsed_line[2].parse::<u16>() else {
            return Err(anyhow::anyhow!("Unable to parse <{}> as K-coordinate #{}", parsed_line[2], counter+1));
        };
        let Ok(value) = parsed_line[3].parse::<T>() else {
            return Err(anyhow::anyhow!("Unable to parse <{}> as value #{}", parsed_line[3], counter+1));
        };
...
mod read_ijk_values_of_type {
    ...
    fn invalid_value_error() {
        ...
        assert!(read_ijk_values::<&[u8], i16>(&mut reader).is_err(), "must be Error");
    }
    #[test]
    fn no_value_error() {
        let s = "\n1 2 3\n\n";
